$(document).ready(function(){
	$("#search-icon").hide();
        ...
        db.collection("organizers").orderBy("name").orderBy("description").limit(20).get().then((querySnapshot) => {
                querySnapshot.forEach((doc) => {
                        cursor = doc;
                        $("#putOrganizersHere").append(createOrganizerCard(doc.data().id, doc.data().name, doc.data().logoURL, doc.data().location));
                });
        });

        $("#loadMoreButton").click(function() {
                db.collection("organizers").orderBy("name").orderBy("description").limit(20).startAfter(cursor).get().then((querySnapshot) => {
                        querySnapshot.forEach((doc) => {
                                cursor = doc;
                                $("#putOrganizersHere").append(createOrganizerCard(doc.data().id, doc.data().name, doc.data().logoURL, doc.data().location));
                        });
